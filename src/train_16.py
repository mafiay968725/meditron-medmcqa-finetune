...
def train_model(lora_rank=8, dropout=0.1, learning_rate=1e-4):
    ...
    num_training_steps = 48000  # 根据实际步数进行调整
    ...
    eval_interval = 1000  # 每多少 global_step 评估一次
    early_stop_start_step = 24000  # 从第几个 step 开始启用 early stopping（约100k数据）
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if global_step >= early_stop_start_step and global_step % eval_interval == 0:
                ...
                print(f"Epoch {epoch + 1}, Global Step: {global_step} , Accuracy: {accuracy:.4f}")
                if accuracy - best_accuracy >= 0.002:
                    best_accuracy = accuracy
                    no_improve_count = 0
                    model.save_pretrained("/home/ubuntu/meditron-medmcqa-finetune/data/train_16/best",
                                              safe_serialization=True)
                    tokenizer.save_pretrained("/home/ubuntu/meditron-medmcqa-finetune/data/train_16/tokenizer")
                else:
                    no_improve_count += 1
    ...
    #训练结束后，调用最好的模型在整个验证集上评估（方便和之前的结果比较）
    base_model_path = "/home/ubuntu/meditron-medmcqa-finetune/models/meditron-7b"
    lora_checkpoint_path = "/home/ubuntu/meditron-medmcqa-finetune/data/train_16/best"
    base_model = AutoModelForCausalLM.from_pretrained(base_model_path,  device_map={"": 0})
    best_model = PeftModel.from_pretrained(base_model, lora_checkpoint_path)
    best_model.to("cuda")
    tokenizer = AutoTokenizer.from_pretrained("/home/ubuntu/meditron-medmcqa-finetune/data/train_16/tokenizer")
    tokenizer.pad_token = tokenizer.eos_token
    # 训练完成后，评估最终的准确率
    accuracy = evaluate_model_accuracy(best_model,tokenizer, dev_dataset)
    save_path = base_dir / "data" / "log" / "train_16.csv"
...
top_configs = [
    {"lora_rank": 16, "dropout": 0.18, "lr": 0.00006}
]
