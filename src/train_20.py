...
def train_model(lora_rank=8, dropout=0.1, learning_rate=1e-4, alpha = 0.5, soft_label_type=0):
    ...
    def format_example(example):
        ...
        if soft_label_type == 0: #(0.75,0.15,0.05)
            ...
            if second_choice is None:
                ...
                soft_label = [0.05, 0.05, 0.05, 0.05]
                soft_label[label_map[first_choice]] = 0.6
                soft_label[label_map[second_choice]] = 0.3
        else: #(0.9,0.1)
            if second_choice is None:
                soft_label = [0.02, 0.02, 0.02, 0.02]
                soft_label[label_map[first_choice]] = 0.94
            else:
                soft_label = [0, 0, 0, 0]
                soft_label[label_map[first_choice]] = 0.9
                soft_label[label_map[second_choice]] = 0.1
    ...
    for epoch in range(epochs):
        ...
        # ✅ epoch 结束时评估
        model.eval()
        save_path = base_dir / "data" / "log" / "train_20.csv"
        if epoch >= 0:
...
top_configs = [
    {"lora_rank": 16, "dropout": 0.1223, "lr": 1.2e-4, "soft_label_type": 0},
    {"lora_rank": 16, "dropout": 0.1223, "lr": 1.2e-4, "soft_label_type": 1},
    {"lora_rank": 16, "dropout": 0.1223, "lr": 1.2e-4, "soft_label_type": 2},
]


label_type_desc = {0: "平衡型", 1: "温和型", 2: "激进型"}
for i, cfg in enumerate(top_configs):
    label_desc = label_type_desc.get(cfg["soft_label_type"], "未知类型")
    print(f"\n🚀 Running Trial {i} with lora_rank={cfg['lora_rank']}, dropout={cfg['dropout']}, lr={cfg['lr']:.6f}")
    print(f"soft_label_type: {label_desc}")

    score = train_model(
        lora_rank=cfg["lora_rank"],
        dropout=cfg["dropout"],
        learning_rate=cfg["lr"],
        soft_label_type=cfg["soft_label_type"]
    )
