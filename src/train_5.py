        ...
        if (i + 1) % accumulation_steps == 0:
            ...
            if global_step % eval_interval == 0:
                ...
                if avg_loss < best_dev_loss:
                    ...
                    model.save_pretrained("/root/meditron-medmcqa-finetune/data/train_5/best")
                    print(f"💾 最优模型已保存，当前 Dev Loss: {avg_loss:.4f}")
                torch.cuda.empty_cache()
                model.train()

    # 每个 epoch 结束后保存一次模型
    save_path = f"/root/meditron-medmcqa-finetune/data/train_5/epoch_{epoch + 1}"
    model.save_pretrained(save_path)
    if epoch == 0:
        tokenizer.save_pretrained("/root/meditron-medmcqa-finetune/data/train_5/tokenizer")
