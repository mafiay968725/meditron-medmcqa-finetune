...
dev_dataloader = DataLoader(dev_subset, batch_size=2, collate_fn=my_collate_fn)
...
accumulation_steps = 8  # 每8个mini-batch累积一次梯度 => 有效batch_size=8×2=16
        ...
        if (i + 1) % accumulation_steps == 0:
            ...
            if global_step % eval_interval == 0:
                ...
                if avg_loss < best_dev_loss:
                    ...
                    model.save_pretrained("/home/ubuntu/meditron-medmcqa-finetune/data/train_12/best", safe_serialization=True)
                    print(f"💾 最优模型已保存，当前 Dev Loss: {avg_loss:.4f}")
                model.train()

    # 每个 epoch 结束后保存一次模型
    save_path = base_dir / "data" / "train_12" / f"epoch_{epoch + 1}"
    model.save_pretrained(save_path, safe_serialization=True)
    if epoch == 0:
        tokenizer.save_pretrained("/home/ubuntu/meditron-medmcqa-finetune/data/train_12/tokenizer")
