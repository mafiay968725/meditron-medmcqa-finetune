...
def train_model(lora_rank=8, dropout=0.1, learning_rate=1e-4, alpha = 0.5, seed = 42):
    ...
    for name, param in model.named_parameters():
        ...
        if "classifier" in name:
            decay_classifier.append(param)  # 分类层独立设 weight decay （可以调高）
    ...
    for epoch in range(epochs):
        ...
        if epoch >=0:
            save_path = base_dir / "data" / "log" / "train_31.csv"
        ...
        if epoch >= 2:
            # try:
            #     save_dir = "/home/ubuntu/meditron-medmcqa-finetune/data/train_31"
...
top_configs = [
  {"lora_rank": 16, "dropout": 0.163, "lr": 0.000065, "alpha": 0.3},
]
seed_list = [42]
