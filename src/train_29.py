...
def train_model(lora_rank=8, dropout=0.1, learning_rate=1e-4, alpha = 0.5, seed = 42):
    ...
    wandb.init(
        ...
        name=f"lr{learning_rate:.6f}_dropout{dropout:.3f}_alpha_{alpha:.3f}_seed{seed}_lrsch_2mlp",
        config={
    ...
    for epoch in range(epochs):
        ...
        if epoch >=0:
            save_path = base_dir / "data" / "log" / "train_29.csv"
        ...
        if epoch >= 3:
            try:
                save_dir = "/home/ubuntu/meditron-medmcqa-finetune/data/train_29"
